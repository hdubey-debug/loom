...
class JsonSandbox(unittest.TestCase):
    ...
    def test_chat_events_rendered_as_json_lines(self):
        bus, state, c = _setup()
        e = ev.chat(sender="user", body="hello @claude_code", addressees=["claude_code"])
        bus.post(e)
...
class DmPrivacyInPrompt(unittest.TestCase):
    def test_dm_to_other_not_in_my_transcript(self):
        bus, state, c = _setup()
        bus.post(ev.chat(sender="user", body="psst gemini", channel="dm:gemini_cli"))
        out = build_prompt("claude_code", trigger_event=None, coordinator=c)
        self.assertNotIn("psst gemini", out)

    def test_dm_to_me_appears_in_my_transcript(self):
        bus, state, c = _setup()
        bus.post(ev.chat(sender="user", body="psst claude", channel="dm:claude_code"))
        out = build_prompt("claude_code", trigger_event=None, coordinator=c)
...
class TriggerAnnotation(unittest.TestCase):
    ...
    def test_optional_label_when_listed_optional(self):
        bus, state, c = _setup()
        e = ev.chat(sender="user", body="hi @loom", addressees=["loom"])
        bus.post(e)
    ...
    def test_dead_letter_trigger(self):
        ...
        bus.post(e)
        _open_default(c, e, "loom")
        dl = ev.dead_letter(42, reason="participant_removed", reroute_to="loom")
        bus.post(dl)
...
class RecencyWindow(unittest.TestCase):
    def test_only_last_n_chat_events_included(self):
        ...
        out = build_prompt("claude_code", trigger_event=None, coordinator=c, n_recent=5)
...
class StableProtocolRules(unittest.TestCase):
    ...
    def test_no_aggressive_participation_framing(self):
        ...
        for phrase in (
            ...
            self.assertNotIn(phrase, out, f"stale phrase still in prompt: {phrase!r}")
...
class TurnCardRendering(unittest.TestCase):
    ...
    def test_wait_for_user_after_renders_stop_line(self):
        bus, state, c = _setup()
        e = ev.chat(sender="user", body="@loom teach", addressees=["loom"])
        bus.post(e)
        from loom.kernel.obligations import plan_with_required
...
class PromptSectionsHook(unittest.TestCase):
    ...
    def _stub_policy(self, sections_fn):
        ...
        class _Stub(ConversationPolicy):
            ...
            def prompt_sections(self, *, state, participant_id, trigger_event):
    ...
    def test_default_returns_no_extra_sections(self):
        ...
        bus, state, c = _setup()
        out = build_prompt("loom", trigger_event=None, coordinator=c, policy=_Bare())
    ...
    def test_custom_sections_render_in_system_preamble(self):
        ...
        policy = self._stub_policy(
            lambda _s, _p, _t: [
                PromptSection(name="taxonomy", text="Use Bloom's taxonomy."),
                PromptSection(name="stance", text="Argue the steelman."),
            ]
        )
        bus, state, c = _setup()
        out = build_prompt("loom", trigger_event=None, coordinator=c, policy=policy)
