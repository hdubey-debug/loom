...
class TestExtendedDialog:
    ...
    @pytest.mark.stress
    def test_50_turn_open_chat_broadcast_no_state_drift(self, multi_turn_session, varied_agents):
    ...
    @pytest.mark.stress
    def test_60_turn_single_responder_only_target_speaks(self, multi_turn_session, varied_agents):
        ...
        for i in range(60):
            ...
            for r in replies:
                if r.sender != "user":
                    any_reply = True
                    assert r.sender == "sr1", f"non-target spoke at turn {i}: {r.sender}"
        assert any_reply
    ...
    def test_80_turn_default_policy_mixed_mention_and_broadcast(
        ...
        for i in range(80):
            if i % 5 == 0:
                replies = room.post_and_wait(f"@dp1 thoughts on item {i}?", timeout=5.0)
                senders = {r.sender for r in replies if r.sender != "user"}
                if senders == {"dp1"}:
                    mentioned_only_dp1 += 1
            else:
                room.post_and_wait(f"open question {i}", timeout=5.0)
        assert mentioned_only_dp1 > 0, "no @-mention turn ever resolved exclusively to dp1"
...
class TestLongHaulInvariants:
    ...
    def test_committed_chat_count_matches_completed_user_turns(
        ...
        completed_closes = [e for e in closed_events if e.body.get("reason") == "completed"]
        agent_chats = [e for e in event_recorder.by_kind("chat") if e.sender == "inv0"]
...
class TestLongHaulMembershipChurn:
    ...
    def test_add_remove_agent_alternation_during_50_turn_session(
        ...
        def churn_send(prompt):
            counter[0] += 1
            return (
                f"churn {counter[0]} reply long enough to bypass "
    ...
    def test_open_chat_3_adds_3_removes_no_actor_thread_leak(
        ...
        def long_send(label: str):
            ...
            def _send(prompt):
                counter[0] += 1
                return (
                    f"{label} reply {counter[0]} long enough "
