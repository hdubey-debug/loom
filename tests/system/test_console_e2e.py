...
class TestSlashCommandsMultiTurn:
    ...
    def test_console_5_chat_then_quit_sees_all_replies_via_notify(self, scripted_console):
        ...
        chat_lines = [ln for ln in script.captured_lines if "rc0" in ln or "rc1" in ln]
        assert len(chat_lines) >= 5

    def test_console_topic_command_changes_state_observed_in_topic_property(self, scripted_console):
        room = _build_room(n_agents=2)
        script = scripted_console(
            [
                "/topic the new topic",
    ...
    def test_console_responder_command_changes_default_responder(self, scripted_console):
        room = _build_room(n_agents=2)
        script = scripted_console(
            [
                "/responder rc1",
    ...
    def test_console_floor_and_release_return_removed_notice(self, scripted_console):
    ...
    def test_console_remove_via_slash_then_post_routes_to_remaining(self, scripted_console):
...
class TestConsoleExitPaths:
    ...
    def test_eof_during_prompt_clean_shutdown_no_thread_leak(self, scripted_console):
        room = _build_room(n_agents=2)
    ...
    def test_keyboard_interrupt_during_prompt_clean_shutdown(self, scripted_console):
        room = _build_room(n_agents=2)
    ...
    def test_unknown_slash_command_does_not_post_as_user_text(
        ...
        user_chats = [e for e in event_recorder.by_kind("chat") if e.sender == "user"]
        ...
        assert any("unknown command" in ln.lower() for ln in script.captured_lines)
...
class TestConsoleNotifyContract:
    ...
    @pytest.mark.timing
    def test_concurrent_actor_writes_serialize_via_notify_lock(self, scripted_console):
        ...
        agent_lines = [ln for ln in captured if any(f"cn{i}" in ln for i in range(4))]
