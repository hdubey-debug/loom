...
def test_bus_snapshot_full(bench, size):
    bus = _seed_bus(size)
    bench(lambda: bus.snapshot(), name=f"bus.snapshot full E={size}", iters=200, inner=10)
...
def test_bus_snapshot_tail_fresh(bench, size):
    ...
    cursor = size - 10
    bench(
        lambda: bus.snapshot(since=cursor),
        name=f"bus.snapshot since=E-10 E={size}",
        iters=200,
        inner=10,
    )
...
def test_bus_snapshot_tail_stale(bench, size):
    ...
    bench(
        lambda: bus.snapshot(since=cursor),
        name=f"bus.snapshot since=E*0.1 E={size}",
        iters=200,
        inner=10,
    )
...
def test_bus_snapshot_audience(bench, size):
    ...
    bus = _seed_bus(size)
    bench(
        lambda: bus.snapshot(audience="bob"),
        name=f"bus.snapshot audience=bob E={size}",
        iters=200,
        inner=10,
    )
