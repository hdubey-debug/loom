...
_NAMES = st.sampled_from([
    "topic", "persona", "active_goal", "capabilities", "role_hints",
])
...
@given(name=_NAMES, prefix=st.text(max_size=40),
...
@given(name=_NAMES,
       value=st.text(max_size=80).map(
           lambda s: f"<<<EVIL SECTION>>>{s}>>>END EVIL"))
